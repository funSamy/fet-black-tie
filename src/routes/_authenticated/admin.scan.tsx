...
import { scanTicket } from "@/lib/admin.functions";
...
type ScanResult = Awaited<ReturnType<typeof scanTicket>>;
...
function ScanPage() {
  ...
  async function handleDetect(codes: IDetectedBarcode[]) {
    ...
    setError(null);
    try {
      const res = await scan({ data: { qrSlug: slug } });
      setLast(res);
    } catch (e) {
  ...
  if (result.outcome === "UNPAID") {
    return (
      <Card tone="error" icon={<XCircle className="h-6 w-6" />} title="Payment not confirmed">
        ...
        <Button onClick={onContinue} className="mt-3 w-full">Scan next</Button>
      </Card>
    );
  }
  // OK
  ...
  if (duplicate) {
    return (
      <Card tone="warn" icon={<AlertTriangle className="h-6 w-6" />} title="Already fully used">
        <p className="text-sm">{result.buyerName} · {result.tierLabel}</p>
