...
import { getOrderStatus, type OrderStatusData } from "@/lib/booking.actions";
...
type Order = OrderStatusData | null;
...
export function ConfirmationPage({ orderId }: { orderId: string }) {
  ...
  if (order.status === "PENDING") {
    return (
      <Center>
        ...
        <h1 className="font-display text-4xl text-foreground mt-1">Confirming your payment</h1>
        <p className="mt-3 max-w-md text-center text-muted-foreground">
          Complete the payment on the Fapshi screen. This page will update automatically once we
    ...
    return (
      <Center>
        ...
        <p className="mt-2 max-w-md text-center text-muted-foreground">
          No worries — your seat hasn't been charged. You can book again from the home page.
        </p>
  ...
  const qrValue = ticket && origin ? `${origin}/checkin/${ticket.qrSlug}` : "";
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        Save this page or screenshot the QR code — it's your entry pass. The QR is scannable{" "}
        {ticket?.slotsTotal ?? 1} time
        {(ticket?.slotsTotal ?? 1) > 1 ? "s" : ""} at the door.
      </p>
    </div>
  );
}
